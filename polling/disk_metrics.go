...
func GetDiskMetrics(sshClient *SSHClient) ([]DiskMetrics, error) {
	...
	for _, line := range lines {
		...
		metrics := DiskMetrics{
			Volume:        ParseResult(fields[0]),
			MountPath:     ParseResult(fields[1]),
			UsedPercent:   ParseResult(strings.TrimSuffix(fields[2], "%")), // Remove trailing '%'
			CapacityBytes: ParseResult(fields[3]),
			UsedBytes:     ParseResult(fields[4]),
			FreeBytes:     ParseResult(fields[5]),
			FreePercent:   100 - ParseResult(strings.TrimSuffix(fields[2], "%")).(float64), // I need to type assert as return type is interface{}
		}
